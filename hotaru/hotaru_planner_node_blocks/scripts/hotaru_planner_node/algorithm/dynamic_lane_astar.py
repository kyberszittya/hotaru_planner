...
class DynamicLanePolygon(PolygonRepresentation):
    ...
    def __init__(self, lane_width, steps_width, horizon_steps, val_scale=1):
            ...
            for j in range(self.steps_width - 1):
                ...
                if d0 < 0.0 and d1 < 0.0 and d2 < 0.0 and d3 < 0.0:
                    ...
                    for oi, o in enumerate(indici):                        
                        d = obstacle_avoidance_euclidean_distance(v[oi][0], 
                                v[oi][1], obstacle.position[0], 
                                obstacle.position[1], val_scale=self.val_scale)
                        self.obstacle_grid[o[0], o[1]] = d
...
class DynamicLaneAstarPlanner(object):
    ...
    def lane_heuristics(self, current_lane):
        ...
        while not fringe.empty():
            ...
            if new_node[1].x == self.environment_representation.horizon_steps - 1:
                ...
                while not p is None:
                    ...
                    weights.appendleft(self.lane_heuristics(p.y)*self.interp_weight_mult+1)                    
                    p = p.parent
            ...
            for n in self.neighbor_indices:                
                ...
                if (new_ind in visited):
                    continue
                h = self.lane_heuristics(new_ind[1])
